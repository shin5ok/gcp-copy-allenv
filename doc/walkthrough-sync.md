...
- **コピー元（移行元）**:
  - ホストプロジェクト: `shingo-ar-sharedhost0926`
  - サービスプロジェクト1 (Debian): `shingo-ar-sharedservice0926-1`
  - サービスプロジェクト3 (Ubuntu): `shingo-ar-sharedservice0926-3`
- **コピー先（移行先）**:
  - ホストプロジェクト: `shingo-ar-dsthost2026052801`
  - サービスプロジェクト1: `shingo-ar-dstshared2026052801`
...
```bash
export DST_HOST_PROJECT_ID="shingo-ar-dsthost2026052801"
export DST_SVC1_PROJECT_ID="shingo-ar-dstshared2026052801"

make prepare-dst ARGS="--project-map shingo-ar-sharedhost0926=$DST_HOST_PROJECT_ID,shingo-ar-sharedservice0926-1=$DST_SVC1_PROJECT_ID -y"
```
- **結果**:
...
```bash
make sync-to-dst ARGS="--project-map shingo-ar-sharedhost0926=$DST_HOST_PROJECT_ID,shingo-ar-sharedservice0926-1=$DST_SVC1_PROJECT_ID -y"
```
- **結果**:
...
```bash
gcloud compute instances describe org-svc1-deb-e2-std4-01 --zone=asia-northeast1-a --project=shingo-ar-dstshared2026052801 --format='value(machineType)'
gcloud compute instances describe org-svc1-deb-n2-std4-01 --zone=asia-northeast1-a --project=shingo-ar-dstshared2026052801 --format='value(machineType)'
```
- **結果**:
