...
- **内部発行コマンド**:
  ```bash
  gcloud compute shared-vpc associated-projects list --project=shingo-ar-sharedhost0926
  ```
- **仕組み**:
...
- **内部発行コマンド**:
  ```bash
  gcloud compute shared-vpc associated-projects list --project=shingo-ar-sharedhost0926 --format="value(id)"
  ```
- **仕組み**:
  取得したサービスプロジェクトIDの一覧（例: `shingo-ar-sharedservice0926-1`, `shingo-ar-sharedservice0926-3`）を変数に格納し、後続のVM探索スコープとして動的に使用します。

### Stage 3: サブネット定義の動的ロード
共有VPC内のサブネット一覧を探索し、それぞれのIPCIDR範囲とデプロイされているリージョンを取得します。
- **内部発行コマンド**:
  ```bash
  gcloud compute networks subnets list --network=shared-vpc --project=shingo-ar-sharedhost0926 --format="json(name, ipCidrRange, region)"
  ```
- **仕組み**:
...
- **内部発行コマンド**:
  ```bash
  gcloud compute instances list --project=shingo-ar-sharedservice0926-1 --format="json(name, zone, machineType, networkInterfaces)"
  ```
- **仕組み**:
  1. **定義外リソースのフィルタリング**:
...
- **内部発行コマンド**:
  ```bash
  gcloud compute instances describe org-svc1-deb-e2-std4-01 --zone=asia-northeast1-a --project=shingo-ar-sharedservice0926-1 --format="json(disks)"
  ```
- **解析・判定ロジック**:
