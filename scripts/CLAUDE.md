...
`sync_env.py` を中心とした実装ルールと、Step ごとの regression 知識（ハマりどころ）。

ルートの `CLAUDE.md`（トークン節約・セキュリティ・Git ルール）も併読すること。
...
- dst のソース受け渡しバケットは `<dst プロジェクト ID>-fn-source-migration`（`steps.serverless_sync.source_bucket` で上書き可）。dst プロジェクト ID がグローバル一意なのでバケット名も一意になる。無ければ関数と同じリージョンに冪等作成する。
  - **バケット作成は `_ensure_fn_source_bucket` で lock + 存在再確認する**。fn-sync は**関数単位で並列**なので、同一 dst プロジェクトの複数関数が同時に「describe（無い）→ create」を実行し、片方が `HTTPError 409: Your previous request to create the named bucket succeeded and you already own it.` で失敗する（regression: shingo-ar-alone081700 の test-1 / www-1）。lock で直列化し、create 失敗時は describe で実在を再確認して存在すれば成功扱いにする。**409 の文字列だけで成功と判定しない**（`Sorry, that name is not available` も 409 で返り、他人のバケットを掴んだまま先へ進んでしまう）。同種の「プロジェクト単位のリソースを flat unit 並列から作る」処理は同じ形にすること。
- **Cloud Functions のビルドには専用 SA を作って `--build-service-account` で明示する（`_ensure_fn_build_service_account`）**。deploy のビルドは Cloud Build が走り、既定のビルド SA は dst の **default compute SA**（`<dst番号>-compute@developer.gserviceaccount.com`）だが、**2024-05-03 以降に作られたプロジェクトは組織ポリシー `constraints/iam.automaticIamGrantsForDefaultServiceAccounts` が既定で強制され、この SA に `roles/editor` が自動付与されない = ロール 0**。移行先は必ず新規プロジェクトなので**毎回該当し**、全関数の deploy が `Could not build the function due to a missing permission on the build service account`（ビルド段階の失敗なのでフラグや env の問題と紛らわしい）で落ちる（regression）。
  - **既定 SA に `roles/cloudbuild.builds.builder` を足す方法は採らない**。既定 SA は **ランタイム ID も兼ねる**（VM / Cloud Run / 全 Function）ので、移行と無関係のワークロードの権限まで広がる。また dst ORG が `cloudbuild.useComputeServiceAccount` を Not Enforced にしている（既定 SA でのビルド禁止）と**付与しても動かない**。公式ドキュメント（functions/docs/building）の 3 択のうち「custom build service account」を採る。
  - dst ごとに `fn-build@<dst>.iam.gserviceaccount.com` を冪等作成し、公式指定の最小 3 ロール `_FN_BUILD_SA_ROLES`（`logging.logWriter` / `artifactregistry.writer` / `storage.objectViewer`）だけ付与する。**`storage.objectViewer` は条件付きにしない** — 公式例の IAM condition は `gcf-v2-sources-*` 等しか許さず、本ツールが使う `<dst>-fn-source-migration` バケットを読めなくなる。
  - 不足ロールの判定は `missing_build_sa_roles`（純粋関数）。**ポリシー取得不能（None）は「全部不足」に倒す**（付与は冪等だが、見送ると deploy が必ず失敗する）。owner/editor 保持なら何も足さない。
  - `--build-service-account` は **`projects/<p>/serviceAccounts/<email>` 形式必須**（email 単体は弾かれる）。**gen1 でも使える**（SDK 580 の v1 surface が `buildServiceAccount` を送る）ので世代分岐は不要。
  - 作成・付与は **dst プロジェクト単位で fn 複製の前にまとめて**行う。関数単位でやると `add-iam-policy-binding` の read-modify-write が etag 競合する（iam_sync と同じ理由）。新規 SA は伝播に数秒かかるので describe で可視化を待つ（VM 用 SA 作成と同じ）。
  - 直後は IAM 伝播が間に合わずビルドが古いポリシーで走ることがあるため、`_FN_BUILD_SA_PROPAGATION_WAIT` 秒待ち、**そのプロジェクトの deploy だけ `retries=1`** にする（用意していないプロジェクトの deploy は再試行しない = 本物のビルドエラーで 45 秒 × 2 を溶かさない）。
  - **dst に SA と権限を足す操作なので必ず可視化する**: WARNING + DIFF の「確認」note（`function_build_sa_granted`）。**この SA は関数の設定 (`buildServiceAccount`) に記録される**ので「移行用の使い捨て」ではなく dst の正式なビルド ID として残す前提で書くこと（消すと再デプロイが壊れる）。用意に失敗したら移行は止めず `function_build_sa_manual`（要対応）で手動コマンド（専用 SA / 既定 SA の両方）を案内する。
  - 設定は `steps.serverless_sync.build_service_account`（利用者管理の SA を使う。ツールは作成も付与もしない）と `grant_build_service_account: false`（自動用意を止める）。`validate_steps_config` が email 形式と bool を実行前に検査する（文字列 `"false"` が真値になって「止めたつもりが作られる」のを防ぐ）。
  - **gcloud 自身が同じ判定をしている**（`command_lib/functions/service_account_util.py: ValidateDefaultBuildServiceAccountAndPromptWarning`）。既定ビルド SA が builder / editor を持たないと `PromptContinue(default=False, cancel_on_no=True)` で中断するが、**`--quiet` だと `console_io.CanPrompt()` が False になり警告すら出ない**まま進んでビルドで落ちる。原因が読めなかったのはこのため。実際の既定ビルド SA は `gcloud builds get-default-service-account --region=<r>` で引ける（古いプロジェクトは `<番号>@cloudbuild`、org policy でも変わるので**決め打ちしない**）。
- **deploy 失敗は `function_deploy_failed` note で DIFF にも出す**。失敗詳細のサマリー行だけだと 1 行に切り詰められて原因が読めず、ビルドログの引き方も分からない。
...
- **既定ランタイム SA（default compute / appspot）は複製しないが、差を DIFF に出す**（`_note_default_runtime_sa_roles` + 純粋関数 `default_runtime_sa_roles`）。除外理由は「dst に同等物が既定で存在する」だったが、**同等なのは存在だけで権限は付かない**（`constraints/iam.automaticIamGrantsForDefaultServiceAccounts` により `roles/editor` の自動付与が無い）。複製方針は変えない（別 ORG に editor を自動で生やさない）代わりに、src の保有ロールと dst の現状を突き合わせて**不足分だけ**「確認」note（`default_runtime_sa_roles`、付与コマンド付き）にする。**dst ポリシーが取れない場合は全件出す**（見落とすより過剰報告）。呼ぶ位置は `_fetch_src_iam_policies` の直後 — その先に「複製対象なし」の早期 return が 2 つあるので、後ろに置くと出なくなる。
- **注記の永続化は `.iam_notes.json`（`_IAM_NOTES_FILE`）**。Step 5.7 は customize (Step 3) / serverless_sync (Step 4.7) より後に走るので、それぞれの注記ファイルに書くと上書きし合う。`load_customize_notes()` が 3 ファイルすべてを読む。書き出しは `_persist_step_notes`（3 者共通のヘルパー）で、処理した src は注記が無くても空リストで上書きする（原因が消えたら注記も消える）。
...
- **Cloud Run サービスの複製元は bulk-export ではなく Step 4.7**（`_ASSET_COVERAGE` の `run.googleapis.com/Service` は `serverless_sync` 所有。詳細は「サーバーレス複製」節）。bulk-export 経由の歴史的経緯として、`export_resource_types: "auto"` 導入前（〜2026-08-15 の `25216ae`）の `--resource-types` 無し経路では **5 件中 asia-northeast1 の 3 件だけ出力**され us-central1 の www-1 / test-1 が落ちていた。`auto` 導入後は **0 件**（`RunService` が `SupportsBulkExport: false` で Kind 一覧に載らない）。**過去ログの「3 件出ている」を今の期待値と取り違えないこと**。Revision は None（履歴。デプロイで再生成）。
