...
# run.services.* / cloudfunctions.functions.* は Step 4.7 (serverless_sync) が
# src の Cloud Run 定義を読むために使う（read-only）。preflight では必須にしていない
# ため、無くても移行は止まらず「一覧取得に失敗 → WARNING + スキップ」になる。
CUSTOM_PERMS="storage.buckets.get,storage.buckets.list,storage.objects.get,storage.objects.list,compute.snapshots.useReadOnly,compute.snapshots.get,compute.snapshots.list,compute.disks.get,compute.disks.list,compute.firewalls.list,compute.networkFirewallPolicies.list,resourcemanager.projects.getIamPolicy,serviceusage.services.list,run.services.get,run.services.list,cloudfunctions.functions.get,cloudfunctions.functions.list"
