...
`architecture/sync-env-flow.md` に実行フロー図を追加しました。`make run` が Step 0 から
Step 99 まで**何を判定して、どこで止まり・どこで複製をやめ・どこで警告だけ出すか**を
6 枚のフロー図で確認できます。GitHub 上でそのまま図が表示されます。

- 起動前ガード（設定不備 / 二重起動 / `gcrane` 不在 / SA・impersonation / コピー元への書込権限 /
  コピー先未作成）の停止条件を図示しています。トラブル時にどのチェックで止まったか当たりを付けられます。
- 「失敗の扱い」一覧で、**実行を止める失敗**と**警告だけ出して先へ進む失敗**の区別が分かります。
- 「ツールが運ばないもの」一覧に、PV データ / Secret の値 / self-managed SSL 証明書など
  **手動移行が前提のもの**をまとめています。

あわせて、実装と突き合わせて次の点を明記しました（利用者の判断に影響するもの）。

- **`make plan` は tfplan を作りません。** dry_run では `terraform init` / `plan` を実行しないため、
  `make run` は毎回ゼロから plan を計算し直します。実行ログの「→ tfplan を生成しました」は
  実体を伴いません。import 可否・コピー先の API 有効化も `make plan` では検証されません。
- **`google_compute_region_disk`（regional PD の PV）は複製されます。** 除外されるのは
  `google_compute_disk` だけです。
- **Step 5（`gce_restore`）は既定で無効です。** config で有効にしていないと、ディスクは
  Terraform 側で除外されたまま誰も作らず、run は成功として終わります。
- **`DIFF.md` が無いことは異常終了の証拠になりません。** Step 99 は `cai_scan` と `bulk_export` の
  両方が有効なときだけ走ります。中断時も失敗一覧そのものはログ末尾に必ず出力されます。
