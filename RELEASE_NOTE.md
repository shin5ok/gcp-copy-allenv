...
`architecture/sync-env-flow.html` を追加しました。ブラウザで開くと、`make run` が
Step 0 から Step 99 まで**何を判定して、どこで止まり・どこで複製をやめ・どこで警告だけ出すか**
を 6 枚のフロー図で確認できます。
...
- 「ツールが運ばないもの」一覧に、PV データ / Secret の値 / self-managed SSL 証明書など
  **手動移行が前提のもの**をまとめています。
- 単体で完結した HTML なので、オフラインでもそのまま開けます（追加インストール不要）。
