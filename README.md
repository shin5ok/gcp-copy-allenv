...
- **コピー先（移行先）プロジェクトID**: コピー先となる新しいGCPプロジェクト群を用意します。
  - コピー先ホスト: (例: `<DST_HOST_PROJECT_ID>`)
  - コピー先サービス1: (例: `<DST_SERVICE_PROJECT_ID_1>`)
  - コピー先サービス3: (例: `<DST_SERVICE_PROJECT_ID_3>`)
...
```bash
make sync-to-dst ARGS="--project-map <SRC_HOST_PROJECT_ID>=【コピー先ホストID】,<SRC_SERVICE_PROJECT_ID_1>=【コピー先サービス1ID】,<SRC_SERVICE_PROJECT_ID_3>=【コピー先サービス3ID】 --dry-run"
```

#### Step 2.3: スナップショットからの完全復元・同期複製の実行 (`make sync-to-dst`)
クローンデプロイを実行します。
```bash
make sync-to-dst ARGS="--project-map <SRC_HOST_PROJECT_ID>=【コピー先ホストID】,<SRC_SERVICE_PROJECT_ID_1>=【コピー先サービス1ID】,<SRC_SERVICE_PROJECT_ID_3>=【コピー先サービス3ID】 -y"
```
> 🚀 **完全同期クローンのメカニズム**
